...
class PathsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create path" do
    assert_difference('Path.count') do
      post paths_url, params: { path: { : @path., description: @path.description, status: @path.status } }
    end
  ...
  test "should update path" do
    patch path_url(@path), params: { path: { : @path., description: @path.description, status: @path.status } }
